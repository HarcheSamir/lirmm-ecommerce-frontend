...
export default function PromotionCreate() {
  ...
  const { createPromotion, isPromotionLoading, isUploading } = useProductStore();
  ...
  const onSubmit = async (data) => {
    const promotionData = { ...data, imageFile: bgImage, productImageFile: productImage };
  ...
  }, [bgImagePreview, productImagePreview]);

  return (
    <div className="flex flex-col">
      <PagesHeader className={'px-4'} title="Créer une Promotion" breadcrumbs={[{ label: 'Tableau de bord', link: '/dashboard' }, { label: 'Promotions', link: '/dashboard/promotions' }, { label: 'Créer' }]} />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col px-4'>
        <section id="sec1" className='flex flex-col lg:flex-row gap-4 lg:gap-8'>
          <div className='w-full lg:w-[30%] lg:shrink-0 flex flex-col mb-4 lg:mb-0'><p className='font-medium text-fblack text-[16px]'>Résumé</p><p className='text-sm font mt-1 text-gray-500'>Définissez les textes de la promotion pour chaque langue.</p></div>
                ...
                {['fr', 'en', 'ar'].map(lang => (
                  <div key={lang} style={{ display: activeLang === lang ? 'block' : 'none' }}>
                    ...
                    <div className='mb-4'>
                     ...
                     <div className='mb-4'>
                      <label htmlFor={`tagline.${lang}`} className="block text-sm font-medium text-gray-700 mb-1">Tagline ({lang.toUpperCase()})</label>
                      <input type="text" id={`tagline.${lang}`} {...register(`tagline.${lang}`)} className="w-full px-3 rounded-[3px] py-2 border border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                    </div>
                     <div>
        ...
        <section id="sec3" className='flex flex-col lg:flex-row gap-4 lg:gap-8'>
            <div className='w-full lg:w-[30%] lg:shrink-0 flex flex-col mb-4 lg:mb-0'><p className='font-medium text-fblack text-[16px]'>Paramètres</p><p className='text-sm font mt-1 text-gray-500'>Configurez le lien, la visibilité et la durée.</p></div>
            <div className="w-full flex flex-col gap-6">
                <div>
                    <label htmlFor="ctaLink" className="block text-sm font-medium text-gray-700 mb-1">Lien CTA (URL)*</label>
                    <input type="url" id="ctaLink" {...register("ctaLink")} placeholder="https://exemple.com/categorie" className={`w-full px-3 rounded-[3px] py-2 border ${errors.ctaLink ? 'border-red-500' : 'border-gray-300'} focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm`} />
                 ...
                 <div>
                    ...
                    <input type="number" id="displayOrder" {...register("displayOrder")} defaultValue="0" className={`w-full px-3 rounded-[3px] py-2 border ${errors.displayOrder ? 'border-red-500' : 'border-gray-300'} focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm`} />
                    {errors.displayOrder && <p className="mt-1 text-xs text-red-600">{errors.displayOrder.message}</p>}
                </div>
                <div>
                    <label htmlFor="expiresAt" className="block text-sm font-medium text-gray-700 mb-1">Date d'expiration (Optionnel)</label>
